...
function initSidebarMenu() {
    jQuery('.menu-arrow > a.menu-link').on('click', function (e) {
        e.preventDefault();

        const $link = jQuery(this);
        const $inner = $link.next('.menu-inner');
        const isOpen = $link.hasClass('open');

        jQuery('.menu-arrow > a.menu-link').not($link)
            .removeClass('open')
            .next('.menu-inner').slideUp();

        $link.toggleClass('open', !isOpen);
        isOpen ? $inner.slideUp() : $inner.slideDown();
    });
}
