                ...
                <div class="card">
                    <div class="card-header border-0 py-3 d-flex justify-content-between align-items-center">
                        <h6 class="card-title mb-0">
                            <i class="fi fi-rr-user-add me-1"></i> Tambah Pengguna
                        </h6>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row g-3 mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="email" class="form-control form-control-sm @error('email') is-invalid @enderror"
                                        id="email" name="email" value="{{ old('email') }}" placeholder="email@kampus.ac.id" required>
                                    @error('email')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
...
@push('js')
    ...
    <script>
        ...
        // Pre-fill old values from server
        @php
            $oldRoles = old('roles', []);
            $oldTags = [];
            foreach ($oldRoles as $name) {
                $oldTags[] = ['value' => $name, 'name' => $name];
            }
        @endphp
        var oldTags = @json($oldTags);
        if (oldTags.length > 0) {
            tagifyRoles.addTags(oldTags);
        }
    </script>
@endpush
